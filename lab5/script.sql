...
INSERT INTO Report(datetime) VALUES (TO_TIMESTAMP('02-APR-23 01.53.05.000000000 AM'));
...
IS
   ...
   t TIMESTAMP;
BEGIN
   SELECT datetime INTO t FROM Report WHERE id = 1;
   fileid := UTL_FILE.FOPEN ('LAB6_DIR', 'report.html', 'W');
   UTL_FILE.put_line (fileid, '<!DOCTYPE html>');
   UTL_FILE.put_line (fileid, '<html>');
   UTL_FILE.put_line (fileid, '<body>');
   FOR l IN (SELECT * FROM Logs WHERE time >= t)
   LOOP
      UTL_FILE.put_line (fileid, '<p>' ||l.type|| ', ' ||l.table_name||'</p>');
   END LOOP;
   UTL_FILE.put_line (fileid, '</body>');
   UTL_FILE.put_line (fileid, '</html>');
   UPDATE Report SET datetime = TO_TIMESTAMP(TO_CHAR(SYSDATE, 'yyyy-mm-dd hh:mi:ss'), 'yyyy-mm-dd hh:mi:ss') WHERE id = 1;
   UTL_FILE.fclose (fileid);
END;

CREATE PROCEDURE GetReportByTime(t IN TIMESTAMP)
IS
   fileid UTL_FILE.file_type;
BEGIN
   fileid := UTL_FILE.FOPEN ('LAB6_DIR', 'report.html', 'W');
